...
use Respinar\PodcastBundle\Security\PodcastPermissions;
use Contao\StringUtil;
use Contao\System;
use Symfony\Component\HttpFoundation\Session\Attribute\AttributeBagInterface;

$GLOBALS['TL_DCA']['tl_podcast_feed'] = array
(
	// Config
	'config' => array
	(
		...
		(
			array('tl_podcast_feed', 'checkPermission'),
			array('tl_podcast_feed', 'generateFeed')
		),
		'oncreate_callback' => array
		(
			array('tl_podcast_feed', 'adjustPermissions')
		),
		...
		(
			array('tl_podcast_feed', 'scheduleUpdate')
		),
		'sql' => array
		(
			'keys' => array
	...
	(
		...
		(
			...
			(
				'href'                => 'act=copy',
				'icon'                => 'copy.svg',
				'button_callback'     => array('tl_podcast_feed', 'copyFeed')
			),
			'delete' => array
			(
				'href'                => 'act=delete',
				...
				'button_callback'     => array('tl_podcast_feed', 'deleteFeed')
			),
			'show' => array
			(
				'href'                => 'act=show',
	...
	'palettes' => array
	(
		'default'                     => '{title_legend},title,alias,language;{podcast_legend},podcast_channel;{config_legend},format,maxItems,feedBase,description;{image_legend:hide},imgSize'
	),
	...
	(
		...
		(
			...
			'save_callback' => array
			(
				array('tl_podcast_feed', 'checkFeedAlias')
			),
		...
		'podcast_channel' => array
		(
			'exclude'                 => true,
			'search'                  => true,
			'inputType'               => 'radio',
			'options_callback'        => array('tl_podcast_feed', 'getAllowedChannels'),
		...
		(
			...
			'load_callback' => array
			(
				array('tl_podcast_feed', 'addFeedBase')
			),
			'sql'                     => "varchar(255) NOT NULL default ''"
		),
		'description' => array
 ...
 */
class tl_podcast_feed extends Backend
{
	/**
	...
	public function checkPermission()
	{
		...
		$GLOBALS['TL_DCA']['tl_podcast_feed']['list']['sorting']['root'] = $root;
		$security = System::getContainer()->get('security.helper');

		// Check permissions to add feeds
		if (!$security->isGranted(PodcastPermissions::USER_CAN_CREATE_FEEDS))
		{
			$GLOBALS['TL_DCA']['tl_podcast_feed']['config']['closed'] = true;
			$GLOBALS['TL_DCA']['tl_podcast_feed']['config']['notCreatable'] = true;
			$GLOBALS['TL_DCA']['tl_podcast_feed']['config']['notCopyable'] = true;
		}

		// Check permissions to delete feeds
		if (!$security->isGranted(PodcastPermissions::USER_CAN_DELETE_FEEDS))
		{
			$GLOBALS['TL_DCA']['tl_podcast_feed']['config']['notDeletable'] = true;
		}
		...
		switch (Input::get('act'))
		{
			...
			case 'delete':
			case 'show':
				if (!in_array(Input::get('id'), $root) || (Input::get('act') == 'delete' && !$security->isGranted(PodcastPermissions::USER_CAN_DELETE_FEEDS)))
				{
			...
			case 'copyAll':
				...
				if (Input::get('act') == 'deleteAll' && !$security->isGranted(PodcastPermissions::USER_CAN_DELETE_FEEDS))
				{
	...
	public function adjustPermissions($insertId)
	{
		...
		if (is_array($arrNew['tl_podcast_feed']) && in_array($insertId, $arrNew['tl_podcast_feed']))
		{
	...
	{
		return System::getContainer()->get('security.helper')->isGranted(PodcastPermissions::USER_CAN_CREATE_FEEDS) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.svg/i', '_.svg', $icon)) . ' ';
	}
	...
	{
		return System::getContainer()->get('security.helper')->isGranted(PodcastPermissions::USER_CAN_DELETE_FEEDS) ? '<a href="' . $this->addToUrl($href . '&amp;id=' . $row['id']) . '" title="' . StringUtil::specialchars($title) . '"' . $attributes . '>' . Image::getHtml($icon, $label) . '</a> ' : Image::getHtml(preg_replace('/\.svg/i', '_.svg', $icon)) . ' ';
	}
	...
	public function getAllowedChannels()
	{
		if ($this->User->isAdmin)
		{
			$objArchive = ChannelModel::findAll();
		}
		else
		{
			$objArchive = ChannelModel::findMultipleByIds($this->User->news);
		}
